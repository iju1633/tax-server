...
- 로그아웃
  - 배포된 상태에선 웹 애플리케이션 서버가 계속 돌아가며 docker container를 내리고 다시 실행시키기 위해선 key값이 필요하기 때문에 웹 애플리케이션 서버를 종료시키고 다시 실행시킨 효과를 주기 위해서 구현
...
- 회원 정보 반환
  ...
  - 예외 상황
    ...
    - 토큰의 유효 기간이 지난 경우
  - 검증 결과 (요청 성공)
    - 코드 200 반환하며 서비스 로그인 아이디, 이름, 가입 일시 반환
...
- 로그아웃
  ...
  - 특징
    - 웹 애플리케이션 서버를 종료시키고 다시 실행시킨 효과를 주기 위해서 구현
  - 검증 결과 (요청 성공)
    - 코드 200 반환
