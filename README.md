...
- 유저의 정보 스크랩 API
- 유저의 환급 정보 반환 API
  - 유저의 스크랩 정보를 바탕으로 유저의 결정세액과 퇴직연금세액공제금액을 계산하여 유저의 이름과 반환하는 API

## 구현 방법 & 검증 결과
- 회원가입
  - 서비스 로직
    - 회원가입에 필요한 정보(서비스 로그인 아이디, 서비스 로그인 비밀번호, 이름, 주민등록번호)가 담긴 DTO를 파라미터로 받아 entity 생성 후, JpaRepository를 상속받은 인터페이스를 활용하여 DB에 저장
  - 특징
...
- 로그인
  ...
  - 특징
    ...
    - 토큰을 응답 DTO에 포함하여 반환
  - 예외 상황
    - 파라미터로 받은 정보로 유저 조회가 안되는 경우 (아이디 존재 x)
    - 계정 정보 불일치 시 (비밀번호 불일치)
- 회원 정보 반환
  - 서비스 로직
  ...
  - 특징
    ...
    - DTO에 entity를 매개변수로 받아 DTO를 반환하는 `static` 메서드를 구현
...
- 유저의 환급 정보 반환
  ...
  - 특징
    ...
    - 15% 등, 소수점이 나올 수 있기에 `double` 형식을 사용하여 `근로소득세액공제금액`, `특별세액공제금액`, `표준세액공제금액`, `퇴직연급세액공제` 계산 후 `결정세액` 도출
